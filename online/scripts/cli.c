...
static const char cmd_flow_add_help[] =
"flow add <ethdev_name> src <ip> dst <ip> proto tcp|udp "
"sport <port> dport <port>\n";

static void
cmd_flow_add(char **tokens,
        uint32_t n_tokens,
        char *out,
        size_t out_size,
        void *obj __rte_unused)
{
        struct flow_params p;
        char *dev_name;
        uint32_t flow_id;
        int status;
        ...
        if (strcmp(tokens[2], "src") != 0) {
                snprintf(out, out_size, MSG_ARG_NOT_FOUND, "src");
                return;
        }
        p.src_ip = tokens[3];

        if (strcmp(tokens[4], "dst") != 0) {
                snprintf(out, out_size, MSG_ARG_NOT_FOUND, "dst");
                return;
        }
        p.dst_ip = tokens[5];

        if (strcmp(tokens[6], "proto") != 0) {
                snprintf(out, out_size, MSG_ARG_NOT_FOUND, "proto");
                return;
        }
        p.proto = tokens[7];

        if (strcmp(tokens[8], "sport") != 0) {
                snprintf(out, out_size, MSG_ARG_NOT_FOUND, "sport");
                return;
        }
        if (parser_read_uint16(&p.src_port, tokens[9]) != 0) {
                snprintf(out, out_size, MSG_ARG_INVALID, "sport");
                return;
        }

        if (strcmp(tokens[10], "dport") != 0) {
                snprintf(out, out_size, MSG_ARG_NOT_FOUND, "dport");
                return;
        }
        if (parser_read_uint16(&p.dst_port, tokens[11]) != 0) {
                snprintf(out, out_size, MSG_ARG_INVALID, "dport");
                return;
        }

        status = flow_add(dev_name, &p, &flow_id);
        if (status) {
                snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
                return;
        }

        snprintf(out, out_size, "Flow added with id %u.\n", flow_id);
}

static const char cmd_flow_del_help[] =
"flow del <ethdev_name> <flow_id>\n";

static void
cmd_flow_del(char **tokens,
        uint32_t n_tokens,
        char *out,
        size_t out_size,
        void *obj __rte_unused)
{
        ...
        if (parser_read_uint32(&flow_id, tokens[2]) != 0) {
                snprintf(out, out_size, MSG_ARG_INVALID, "flow_id");
                return;
        }

        status = flow_del(dev_name, flow_id);
        if (status) {
                snprintf(out, out_size, MSG_CMD_FAIL, tokens[0]);
                return;
        }

        snprintf(out, out_size, "Flow %u deleted.\n", flow_id);
}
...
cli_process(char *in, char *out, size_t out_size, void *obj)
{
	...
	if (strcmp(tokens[0], "flow") == 0) {
		if (n_tokens >= 2 && strcmp(tokens[1], "add") == 0) {
				/* shift tokens so cmd_flow_add sees tokens[0]=dev_name style layout */
				cmd_flow_add(&tokens[1], n_tokens - 1, out, out_size, obj);
				return;
		}
		if (n_tokens >= 2 && strcmp(tokens[1], "del") == 0) {
				cmd_flow_del(&tokens[1], n_tokens - 1, out, out_size, obj);
				return;
		}
		snprintf(out, out_size, "\n%s%s\n", cmd_flow_add_help, cmd_flow_del_help);
		return;
	}
